...
int main() {
    ...
    // Generate board (same as before)
    State game;
    std::vector<int> order(361);
    for (int i = 0; i < 361; ++i) order[i] = i;
    std::mt19937 rng{45}; // Fixed seed for reproducibility
    ...
    // === FIXED: 2-thread pool (no per-iteration thread creation) ===
    std::atomic<int> counter{0};
    std::bitset<State::OFFSET> results[2][ROUNDS];  // 0 = black, 1 = white
    ...
    std::cout << "Parallel (2 pooled threads) | Stones: " << placed
              << " | Rounds: " << ROUNDS
              << " | Total: " << ns << " ns"
